...
class MockServer(threading.Thread):
    def __init__(self, port, expected_queue, variant="none", standalone=False, endian='big', header_bytes=2):
        super().__init__()
        self.port = port
    ...
    def run(self):
        print(f"[MOCK] Listening on port {self.port} (Endian: {self.endian})...")
    ...
    def handle_connection(self, conn):
        try:
            conn.settimeout(5.0)
            fmt_char = ">" if self.endian == 'big' else "<"
            len_fmt = f"{fmt_char}H" if self.header_bytes == 2 else f"{fmt_char}I"
            header_len = self.header_bytes

            while self.running:
                # 1. Read Length 
                len_bytes = conn.recv(header_len)
                if not len_bytes:
                    break
                print(f"[DEBUG] Mock Recv Len Bytes: {len_bytes.hex()}")
