...
def main():
    ...
    parser.add_argument("--rewrite-endian", choices=['big', 'little'], help="Rewrite 2-byte header to specific endianness")
        ...
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                ...
                for i, payload in enumerate(payloads):
                    ...
                    # Header Rewriting for Endian Mismatch
                    if args.rewrite_endian:
                        import struct
                        # Assume 2-byte header
                        # Read as Big Endian (Standard Network/PCAP usually)
                        original_len = struct.unpack(">H", payload[:2])[0]
                        
                        if args.rewrite_endian == 'little':
                            new_header = struct.pack("<H", original_len)
                        else:
                            new_header = struct.pack(">H", original_len)
                            
                        print(f"  [DEBUG] Rewriting Header: From {payload[:2].hex()} To {new_header.hex()} (Len={original_len})")
                        payload = new_header + payload[2:]
