...
try:
    ...
    for m in data:
        if m.get('name') == '$metric_name':
            ...
            if attrs.get('$attr_key') == '$attr_val':
                print(m.get('value', 0))
                found = True
                break
    if not found:
...
make -C "${ROOT_DIR}" build > /dev/null || fail "Build failed"
...
if python3 "${BASE_DIR}/iso8583_tool.py" --e2e --host 127.0.0.1 --port $ISO_PORT --encoding ascii --endian big --count 1; then
...
verify_metrics "fluxrig_rack_latency_seconds" 1 "Dynamic ASCII-BE" "1" "direction=inbound"
...
if python3 "${BASE_DIR}/iso8583_tool.py" --e2e --host 127.0.0.1 --port $ISO_PORT --encoding ascii --endian little --count 1; then
...
if python3 "${BASE_DIR}/iso8583_tool.py" --e2e --host 127.0.0.1 --port $ISO_PORT --encoding bcd --endian big --count 1; then
...
if python3 "${BASE_DIR}/iso8583_tool.py" --e2e --host 127.0.0.1 --port $ISO_PORT --encoding bcd --endian little --count 1; then
...
if python3 "${BASE_DIR}/iso8583_tool.py" --e2e --host 127.0.0.1 --port $ISO_PORT --encoding ascii --endian big --variant visa --count 1; then
