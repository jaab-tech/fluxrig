...
if ! python3 "${ROOT_DIR}/test/utils/verify_trace.py" "${WORK_DIR}/rack/logs/rack.log"; then
...
if command -v duckdb >/dev/null 2>&1; then
  python3 "${ROOT_DIR}/test/utils/compare_logs.py" "${WORK_DIR}" || fail "Log Parity Check Failed"
else
